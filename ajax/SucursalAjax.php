<?php
	...
	switch ($_GET["op"]) {
		...
		case "listSucursalEmp":
            ...
            if ($estadoAdmin == "S") {
            	echo '<tr>
            		<td><button type="button" onclick="AccederSuperAdmin('.$idempleado.',\''.$idusuario.'\',\''.$idsucursal.'\',\''.$estadoAdmin.'\',\''.$empleado.'\',\''.$tipo_documento.'\',\''.$direccion.'\',\''.$telefono.'\',\''.$foto.'\',\''.$email.'\',\''.$login.'\',\''.$mnu_almacen.'\',\''.$mnu_compras.'\',\''.$mnu_ventas.'\', \''.$mnu_pago.'\',\''.$mnu_mantenimiento.'\',\''.$mnu_seguridad.'\',\''.$mnu_consulta_compras.'\',\''.$mnu_consulta_ventas.'\',\''.$mnu_admin.'\',\''.$regConf->logo.'\')" class="btn btn-success pull-left">Acceder</button></td>
